...
#### 闭包
> 参考知乎[回答](https://www.zhihu.com/question/24084277)，可归结为以下几点
1. 一个依赖于自由变量的函数
2. 处在含有这些自由变量的一个外围环境
...
#### 重要特征
+ 不需要声明参数类型
- 可以有0或多个参数，一个参数无需定义圆括号
* 表达式主题只有一条语句，则无需{}；若是多条语句，则需用{}形成代码块

#### 常用方法

1. 实现Runnable  
用lambda表达式替换[匿名类](https://www.cnblogs.com/progor/p/8644634.html)，而实现Runnable接口是匿名类最好的示例；
2. 实现函数式接口  
只有函数式接口，才可以转化为lambda表达式
3. 对列表进行迭代  
使用forEach()方法，可以迭代列表中所有对象，并对其中的对象应用lambda表达式，或是方法引用(本质还是lambda表达式)
